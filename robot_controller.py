...
class RobotController(object):
    ...
    #define the turning left operator
    def turn_left(self):
        if self.moves < self.max_moves:
            self.moves += 1
            self.direction = (self.direction - 1) % 4

    #define the turning right operator
    def turn_right(self):
        if self.moves < self.max_moves:
            self.moves += 1
            self.direction = (self.direction + 1) % 4
    ...
    #define a method to detect a target and update te matrix accordingly
...
#define a class to generate functions depending on the number of input elements
class Prog(object):
...
#define a function to create the toolbox and add the primitives
def create_toolbox():
    global robot, pset
